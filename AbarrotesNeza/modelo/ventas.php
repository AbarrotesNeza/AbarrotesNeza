...
class claseVentas{
	...
	function consultaPendientes($base){
		$registros=mysqli_query($base->conectarBD(),"SELECT v.*, u.nombre_us from ventas v, usuario u WHERE situacion=1 AND u.id_us=v.id_us")or die(mysqli_error($base->conectarBD()));
		$base->desconectarBD();
		return $registros;
	}

	function consultaRealizadas($base){
		$registros=mysqli_query($base->conectarBD(),"SELECT v.*, u.nombre_us from ventas v, usuario u WHERE situacion=2 AND u.id_us=v.id_us")or die(mysqli_error($base->conectarBD()));
		$base->desconectarBD();
		return $registros;
	}
	function consultaCanceladas($base){
		$registros=mysqli_query($base->conectarBD(),"SELECT v.*, u.nombre_us from ventas v, usuario u WHERE situacion=3 AND u.id_us=v.id_us")or die(mysqli_error($base->conectarBD()));
		$base->desconectarBD();
		return $registros;
	}
